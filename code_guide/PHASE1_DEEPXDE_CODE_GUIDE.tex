...
센서 데이터는 \texttt{main\_deepxde.py} 안에 직접 정의된 \texttt{synthetic\_theta\_no\_conduction()}으로 만든다. 다만 DeepXDE의 \texttt{PointSetBC}는 NumPy 배열을 받기 때문에 \texttt{torch.Tensor}를 NumPy로 변환한다.

Mac에서 PyTorch가 MPS backend를 쓰면 tensor가 \texttt{mps:0} 장치에 올라갈 수 있다. 이 상태에서는 바로 \texttt{.numpy()}를 호출할 수 없으므로, 먼저 \texttt{.detach().cpu()}로 계산 그래프에서 분리하고 CPU로 옮긴 뒤 NumPy 배열로 바꾼다.
